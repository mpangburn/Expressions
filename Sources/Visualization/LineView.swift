...
/// A view containing a line drawn across its bounds.
public class LineView: UIView {
    ...
    public var direction = Direction.vertical { didSet { setNeedsDisplay() } }
    ...
    override public func draw(_ bounds: CGRect) {
        ...
        let (start, end) = lineEndPoints(basedOn: direction)
        path.move(to: start)
        path.addLine(to: end)
    ...
    private func lineEndPoints(basedOn direction: Direction) -> (start: CGPoint, end: CGPoint) {
        let start: CGPoint
        let end: CGPoint
        switch direction {
        case .vertical:
